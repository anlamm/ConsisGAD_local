...
from models_pe import simpleGNN_MR
...
from modules.aux_mod import fixed_augmentation, RandomWalkSampler
...
def simple_train_epoch(epoch, model, loss_func, graph, homograph, label_loader, unlabel_loader, optimizer, augmentor, args):
    model.train()
    num_iters = args['train-iterations']
    
    sampler, attn_drop, ad_optim, rw_sampler = augmentor
    ...
    for idx in range(num_iters):
        ...
        _, _, s_blocks = fixed_augmentation(graph, label_idx.to(args['device']), sampler, aug_type='none')
        rw_graph = rw_sampler.sample(graph, label_idx.to(args['device']))

        s_pred = model(s_blocks, rw_graph)
        s_target = s_blocks[-1].dstdata['label']
...
def get_model_pred(model, graph, homograph, data_loader, sampler, rw_sampler, args):
    model.eval()
    ...
    with torch.no_grad():
        for node_idx in data_loader:
            if args['ego']:
                ...
                rw_graph = rw_sampler.sample(rw_graph, node_idx.to(args['device']))
            
            pred = model(blocks, rw_graph)
...
def val_epoch(epoch, model, graph, homograph, valid_loader, test_loader, sampler, rw_sampler, args):
    valid_dict = {}
    valid_pred, valid_target = get_model_pred(model, graph, homograph, valid_loader, sampler, rw_sampler, args)
    ...
    test_dict = {}
    test_pred, test_target = get_model_pred(model, graph, homograph, test_loader, sampler, rw_sampler, args)
...
def run_model(args):
    ...
    homograph = dgl.to_homogeneous(graph, ndata=['feature', 'label'])
    ...
    graph = graph.to(args['device'])
    homograph = homograph.to(args['device'])
    ...
    rw_sampler = RandomWalkSampler(fanout=args['rw_fanout'], walk_length=args['rw_length'])
    ...
    augmentor = (sampler, attn_drop, ad_optim, rw_sampler)

    task_loss = nll_loss
    
    best_val = sys.float_info.min
    for epoch in range(args['epochs']):
        train_epoch(epoch, my_model, task_loss, graph, homograph, label_loader, unlabel_loader, optimizer, augmentor, args)
        val_results, test_results = val_epoch(epoch, my_model, graph, homograph, valid_loader, test_loader, sampler, rw_sampler, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rw_fanout', type=int, default=100)
    parser.add_argument('--rw_length', type=int, default=5)
    ...
    args['rw_fanout'] = args0.rw_fanout
    args['rw_length'] = args0.rw_length
